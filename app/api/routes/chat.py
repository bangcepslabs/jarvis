...
async def chat(request: ChatRequest, chat_service: ChatService = Depends(get_chat_service)) -> ChatResponse:
    ...
    return ChatResponse(reply=response.reply, tool_calls=response.tool_calls, pending_action=response.pending_action, presentation_hint=response.presentation_hint)
