...
def transcribe(model: Any, path: Path, vad_filter: bool, language: str, beam_size: int) -> dict[str, Any]:
...
def benchmark_config(
    ...
    cold = transcribe(model, path, vad_filter, language, beam_size)
    warm_runs = [transcribe(model, path, vad_filter, language, beam_size) for _ in range(runs)]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--expected", help="Expected text for a simple normalized CER comparison.")
...
def main() -> int:
    ...
    if args.runs < 1 or any(thread <= 0 for thread in args.cpu_threads):
            ...
            for vad_filter in args.vad:
                for cpu_threads in args.cpu_threads:
                    ...
                    if args.expected is not None:
                        result["cer"] = round(character_error_rate(transcript, args.expected), 3)
    ...
    for result in results:
        print(
            ...
            f"{result['cpu_threads']} | {result['model_load_ms']}ms | "
