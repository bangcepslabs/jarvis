...
class JarvisApiClient {
  ...
  Future<Uint8List> synthesize(String text, {AvatarPresentationHint? presentationHint, String? voiceProfileId}) async { final body = <String,dynamic>{'text':text,'language':'ko', ...?presentationHint == null ? null : {'presentation_hint': presentationHint.toJson()}, ...?voiceProfileId == null ? null : {'voice_profile_id': voiceProfileId}}; final r = await _client.post(Uri.parse('$baseUrl/api/tts/synthesize'), headers: _headers({'content-type':'application/json'}), body: jsonEncode(body)).timeout(const Duration(seconds: 60)); if (r.statusCode != 200) throw Exception(r.body); return r.bodyBytes; }
