...
class STTService:
    def __init__(self, provider: STTProvider, max_file_mb: int = 20, timeout_seconds: float = 60.0, max_concurrent_requests: int = 1, sample_capture: STTSampleCapture | None = None) -> None:
        ...
        self._sample_capture = sample_capture
    ...
    async def transcribe(self, audio: bytes, filename: str | None = None) -> TranscriptionResult:
        ...
        result = result.model_copy(update={"timings_ms": timings_ms})
        if self._sample_capture is not None:
            self._sample_capture.capture(audio, result)
