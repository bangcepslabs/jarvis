...
void main() {
  ...
  test('arms engine once and forwards one detection', () async {
  ...
  test('duplicate detection during suspended turn is ignored', () async {
    await controller.arm();
    engine.emit(const WakeWordDetected(name: 'Hey Jarvis', score: .8));
  ...
  test('unavailable engine does not prevent controller lifecycle', () async {
  ...
  test('dispose does not start engine again', () async {
    controller.dispose();
    await controller.arm();
  ...
  test('old async arm completion cannot overwrite a later suspend', () async {
