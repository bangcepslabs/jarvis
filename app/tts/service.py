...
class TTSService:
    def __init__(self, provider: TTSProvider, max_text_chars: int = 1000, timeout_seconds: float = 60.0, max_concurrent_requests: int = 1) -> None:
    ...
    async def synthesize(self, text: str, language: str = "ko", speaker: int = 0, speed: float = 1.0) -> SynthesisResult:
        ...
        async with self._semaphore:
            try:
                result = await asyncio.wait_for(self._provider.synthesize(text.strip(), language, speaker, speed), timeout=self._timeout_seconds)
